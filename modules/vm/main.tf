...
resource "azurerm_virtual_machine" "main" {
  ...
  os_profile {
    ...
    admin_username = data.vault_generic_secret.secret_data.data["admin_username"]
    admin_password = data.vault_generic_secret.secret_data.data["admin_password"]
  }
...
resource "null_resource" "ansible" {
  ...
  provisioner "remote-exec" {
    ...
    connection {
      type     = "ssh"
      user     = data.vault_generic_secret.secret_data.data["admin_username"]
      password = data.vault_generic_secret.secret_data.data["admin_password"]
    ...
    inline = [
      ...
      "ansible-pull -i localhost  -U https://github.com/manojtalluri50/roboshop-ansible roboshop.yaml -e app_name=${var.component} -e ENV=${var.env} -e token=${var.vault_token}"
    ]
  }
}